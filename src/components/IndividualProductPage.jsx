...
const IndividualProductPage = ({ products, addToCart, modal, card }) => {
    ...
    const handleCart = async (product) => {
        addToCart(product, mySize.current, myColor.current)
    }
                ...
                <div className="container" style={{ marginTop: "10px" }}>
                    <div className="d-flex jusitfy-content-center align-items-center gap-3"
                        ...
                        <div className="product-details" style={{ width: "50%", padding: "10px" }}>
                            ...
                            {card.map((c) => c.id === parseFloat(id) && modal ? <ModalBox /> : null)}
                            <h5>${product.price}</h5>
                            <button className="btn btn-lg btn-dark"
                                onClick={() => handleCart(product)} style={{ width: "100%" }}>ADD TO CART</button>
                        </div>
                    </div>
